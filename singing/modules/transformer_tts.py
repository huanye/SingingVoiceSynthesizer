...
class TransformerEncoder(nn.Module):
    def __init__(self, arch, embed_tokens, last_ln=True):
        ...
        self.note_dur_embed_tokens = Embedding(500, self.hidden_size, self.padding_idx)
        self.note_pitch_embed_tokens = Embedding(100, self.hidden_size, self.padding_idx)

    def forward_embedding(self, src_tokens, note_dur, note_pitch):
        # embed tokens and positions
        embed = self.embed_scale * self.embed_tokens(src_tokens)
        note_dur_embed = self.embed_scale * self.note_dur_embed_tokens(note_dur)
        note_pitch_embed = self.embed_scale * self.note_pitch_embed_tokens(note_pitch)
        positions = self.embed_positions(src_tokens)
        # x = self.prenet(x)
        x = embed + note_dur_embed + note_pitch_embed + positions
        x = F.dropout(x, p=self.dropout, training=self.training)
        return x, embed

    def forward(self, src_tokens, note_dur, note_pitch):
        """
        ...
        """
        x, encoder_embedding = self.forward_embedding(src_tokens, note_dur, note_pitch)
...
class TransformerTTS(nn.Module):
    ...
    def build_embedding(self, dictionary, embed_dim):
        ...
        emb = nn.Embedding(num_embeddings, embed_dim, self.padding_idx)
        return emb
    ...
    def forward_encoder(self, src_tokens, note_dur, note_pitch, spk_ids=None, *args, **kwargs):
        return self.encoder(src_tokens, note_dur, note_pitch)
    ...
    def forward(self, src_tokens, note_dur, note_pitch, prev_output_mels, target_mels, spk_ids=None, *args, **kwargs):
        encoder_outputs = self.forward_encoder(src_tokens, note_dur, note_pitch, spk_ids)
