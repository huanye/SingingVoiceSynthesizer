...
class TransTTSDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        sample = {
            ...
            "target": spec,
            "note_dur": torch.LongTensor(item['note_dur']),
            "note_pitch": torch.LongTensor(item['note_pitch']),
        }
        # if self.num_spk > 1:
    ...
    def collater(self, samples):
        ...
        note_durs = note_durs.index_select(0, sort_order)
        note_pitchs = note_pitchs.index_select(0, sort_order)
...
class TransformerTtsTask(BaseTask):
    ...
    def _training_step(self, sample, batch_idx, _):
        ...
        output, _ = self.model(input, note_dur, note_pitch, prev_output_mels, target)
    ...
    def validation_step(self, sample, batch_idx):
        ...
        prev_output_mels = sample['prev_output_mels']
        target = sample['targets']
        output, attn_logits = self.model(input, note_dur, note_pitch, prev_output_mels, target)
        outputs = {}
    ...
    def infer_batch(self, sample):
        ...
        encoder_outputs = model.forward_encoder(input, note_dur, note_pitch)
